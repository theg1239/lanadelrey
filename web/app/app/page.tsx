...
function DropZone({ onFile, processing, file, status, }: {
    ...
    return (<div onDragOver={(e) => { e.preventDefault(); setOver(true); }} onDragLeave={() => setOver(false)} onDrop={(e) => {
            ...
            <input ref={ref} type="file" accept={accept} onChange={(e) => { const f = e.target.files?.[0]; if (f)
        onFile(f); }} className="hidden"/>
...
function LeftSidebar() {
    ...
    return (<div className="w-52 shrink-0 flex flex-col border-r border-border/40 bg-sidebar">
            ...
            <nav className="flex-1 p-3 space-y-0.5">
                {navItems.map((item) => (<button key={item.label} className={cn("w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors text-left", item.active
                ...
                : "text-sidebar-foreground/50 hover:text-sidebar-foreground/80 hover:bg-sidebar-accent/10")}>
                        ...
                        <div className={cn("w-1.5 h-1.5 rounded-full shrink-0", item.active ? "bg-primary" : "bg-sidebar-foreground/20")}/>
                        {item.label}
                    </button>))}
            </nav>
...
export default function AppPage() {
    ...
    return (<div className="h-dvh w-dvw flex overflow-hidden bg-background">
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                "confidence scoring",
                "speaker diarization",
            ].map((cap) => (<div key={cap} className="flex items-center gap-2">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex flex-wrap gap-1">
                                                {["wav", "mp3", "ogg", "flac", "webm", "mp4"].map((fmt) => (<span key={fmt} className="font-mono text-[8px] tracking-wider uppercase text-muted-foreground/40 border border-border/40 rounded-md px-2 py-0.5 bg-card/30">
                                                        {fmt}
                                                    </span>))}
                                            </div>
                                        </div>
                ...
                { k: "accuracy", v: "99.1%" },
                { k: "languages", v: "12" },
            ].map((stat) => (<div key={stat.k} className="flex items-center justify-between">
                                ...
                                {hasResult && (<motion.div key="sidebar-results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4 space-y-5">
                                        ...
                                        {result.insights?.entities?.length > 0 && (<div className="space-y-1.5">
                                                <span className="font-mono text-[9px] tracking-[0.2em] uppercase text-muted-foreground/40 block font-medium">
                                                    entities
                                                </span>
                                        ...
                                        {result.insights?.obligations?.length > 0 && (<div className="space-y-1.5">
                                                ...
                                                <div className="space-y-2">
                                                    {result.insights.obligations.map((ob, i) => (<p key={i} className="text-[11px] leading-[1.6] text-foreground/60 font-serif italic pl-2 border-l-2 border-primary/30">
                                                            {ob.text}
                                                        </p>))}
                                                </div>
                                            </div>)}
                                        ...
                                        {activeSegment !== null && result.segments[activeSegment] && (<motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="space-y-1.5 pt-3 border-t border-border/30">
                                                <span className="font-mono text-[9px] tracking-[0.2em] uppercase text-muted-foreground/40 block font-medium">
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    {result.segments[activeSegment].confidence != null && (<p className="font-mono text-[10px] text-muted-foreground/50 tabular-nums">
