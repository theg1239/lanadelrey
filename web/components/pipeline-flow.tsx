...
const stages = [
    ...
    },
    {
        ...
        description: "Fintech-aware ASR with domain vocabulary and confidence scoring.",
        bullets: [
    ...
    },
    {
        ...
        description: "Extract structured intelligence from every financial conversation.",
        bullets: [
...
export function PipelineFlow() {
    return (<section id="pipeline" className="py-24 sm:py-32 px-6">
            <div className="max-w-6xl mx-auto">
                
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6 }} className="text-center mb-16 space-y-3">
            ...
            return (<motion.div key={stage.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5, delay: i * 0.15 }}>
                                <Card className="relative h-full p-6 bg-card/50 border-border hover:border-primary/30 transition-all duration-300 group">
                                    ...
                                    <ul className="space-y-1.5">
                                        {stage.bullets.map((bullet) => (<li key={bullet} className="flex items-start gap-2 text-xs text-muted-foreground/80">
                                    ...
                                    {i < stages.length - 1 && (<div className="hidden md:flex absolute -right-5 top-1/2 -translate-y-1/2 z-10 items-center justify-center w-8 h-8 rounded-full bg-background border border-border">
